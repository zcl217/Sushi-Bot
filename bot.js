...
client.on('message', function(message) {
	...
	if (message.author.id.localeCompare("276207143961755648") !== 0){
		...
		//commands that don't require editing the original message
		switch (message.content.toLowerCase()){
			...
			case "!trophies":
				displayTrophies(message);
				break;
			
			//displays the number of coins a user has
			case "!coins":
				coins(message);
				break;
		...
		if (message.content.toLowerCase().substring(0, 14) === "!trophiesother"){
...
function displayTrophies(message){
	message.channel.startTyping();
...
function displayTrophiesOther(message){
...
function giveTrophy(message, values){
	console.log(values);
...
function removeTrophy(message, values){
	let trophy = "";
	for (let a = 0; a < values.length-2; a++){
		trophy += values[a+2];
	}
...
function giveCoins(message, values){
	console.log(values);
...
function removeCoins(message, values){
	console.log(values);
